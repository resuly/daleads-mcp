...
2. Read `coverage.official_register`. `checked_with_findings`, `checked_clear`,
   `not_integrated` and `unavailable` are different conclusions.
3. Keep on-site findings separate from nearby evidence. Historical business,
...
4. Treat an error or unavailable source as incomplete, never clean. A high score
   or checked-clear register is not evidence that the soil is uncontaminated.
5. Preserve caveats and response-level attribution in reports or quoted output.
